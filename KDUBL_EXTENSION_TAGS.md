...
>
> **当前覆盖国家**：Saudi Arabia (SA) · Hungary (HU)
...
### 3.1 `kdubl:InvoiceDocumentReference` — 红票开具原因

用于 **381 红票**，放在 `kdubl:PiaozoneExtension` 下：

```xml
<kdubl:InvoiceDocumentReference>
    <kdubl:DocumentIssuanceReasonCode>KSA-CN01</kdubl:DocumentIssuanceReasonCode>
    <kdubl:DocumentIssuanceReason>Return of goods - items damaged in transit</kdubl:DocumentIssuanceReason>
</kdubl:InvoiceDocumentReference>
```

| 子标签 | 含义 | PUF/ZATCA 对应 |
|--------|------|----------------|
| `DocumentIssuanceReasonCode` | 开具红票的原因代码 | KSA 规范中的原因代码 |
| `DocumentIssuanceReason` | 原因描述文本 | KSA-10 `BillingReferenceExtension/Note` |
...
|----------------------|------|---------|------|
| `SubInvoiceTypeCode` | SA、HU | 所有文件 | 发票类型代码，写入 PUF `InvoiceTypeCode/@name`。SA 用 7 位事务代码，HU 用 `NORMAL`/`SIMPLIFIED`/`AGGREGATE` |
...
| `kdubl:InvoiceDocumentReference` | SA | 红票 381 | KSA-10 红票原因块 |
| `kdubl:DocumentIssuanceReasonCode` | SA | 红票 381 | KSA 原因代码 |
| `kdubl:DocumentIssuanceReason` | SA | 红票 381 | KSA 原因文本 |
